...
-- CSV Vorwahl + Ländervorwahl berichtigen
...
-- Tabelle 3: modules
-- Das sind die Inhalte der Tabelle (IDs bereits vorhanden!):
SELECT DISTINCT lieblingsmodul,
                beschreibung,
                anzahl_tage
FROM students_1nf
ORDER BY lieblingsmodul;

DROP TABLE IF EXISTS modules;

CREATE TABLE modules AS
	(SELECT DISTINCT lieblingsmodul AS modul_id,
	                 beschreibung,
	                 anzahl_tage
	 FROM students_1nf
	 ORDER BY lieblingsmodul);
